...
缓存是整套音频系统的性能核心，直接决定演奏时是否卡顿、音画是否同步。

**为什么必须缓存**：游戏里声音是「按判定实时触发」的——一首曲子中往往每秒有多个音符同时落到判定线，手动模式下每次命中还会因 PERFECT / GREAT / GOOD 分三档各调一次 `get_note_sound()`。若每次发声都现场 `pygame.mixer.Sound(filepath)` 去**读盘 + 解码 WAV**，解码是 CPU 密集操作（把文件解成可被声卡播放的采样缓冲），会引入毫秒级延迟并抢占主线程，导致掉帧、声音与画面命中特效错位。缓存把「解码」这一最贵的一步从热路径上彻底拿掉。

**`SAMPLE_SOUND_CACHE` 的工作方式**（定义 game.py:138；写入 `load_piano_sample` game.py:297）：
...
- **键 = `音名_力度`**：同一音名按力度（vel 4 / 8 / 12 / 16）分出不同条目，对应真实钢琴的不同触键力度采样；
- **上限 500 条 + LRU 淘汰**（game.py:331）：缓存超过 500 时清掉最早一半。一首曲子用到的「音名 × 力度」组合极少超过 500，正常情况下几乎不触发淘汰；即便触发也只是丢最旧条目，下次用到再临时解码补回，不影响正确性；
- **回退链**（见 §4）：某音不在缓存且采样缺失时，自动退到同音名其它八度 / `C4` / 第一个采样，保证首发也能出声并补入缓存。

**预加载把缓存填满**（game.py:1030，`preload_all_samples`，见 §8）：启动 Loading 期间就把全部 88 采样 + 多力度层一次性解码进 `SAMPLE_SOUND_CACHE`。于是正式演奏阶段**几乎 100% 命中缓存、直接返回 Sound**，把「启动时的 Loading 等待」换成「运行时稳定帧率」——典型的以空间换时间。

**跨曲复用**：缓存**不随切歌 / 重玩清空**（只有 `AudioManager.stop_all()` 清播放通道，不清 `SAMPLE_SOUND_CACHE`）。因此同一局里换曲、重玩、Demo 来回切，都直接复用已解码的采样，整场只需付一次解码成本。

> 一句话总结：采样文件在 `~/Piano`（运行时读取、不进仓库），而 `SAMPLE_SOUND_CACHE` 是其**运行时内存镜像**；预加载 + 字典缓存让「读盘解码」只在启动时发生一次，演奏热路径上只剩 O(1) 的字典查找。
