...
# 背景图统一从仓库 Assets/bg_song.jpg 读取，运行时不再联网下载（见 load_bg_images）
...
def load_bg_images():
    """加载仓库内的背景图 Assets/bg_song.jpg，所有界面共用同一张；缺失则用渐变兜底。"""
    bg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'Assets', 'bg_song.jpg')
    bg_images = {}
    img = None
